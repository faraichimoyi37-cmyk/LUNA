...
export const countries: Country[] = [
  ...
  { name: 'Guernsey', code: 'GG', dial: '+44-1481' },
  ...
  { name: 'Montserrat', code: 'MS', dial: '+1-664' },
  ...
  { name: 'Northern Mariana Islands', code: 'MP', dial: '+1-670' },
  ...
  { name: 'Sint Maarten', code: 'SX', dial: '+1-721' },
  ...
  { name: 'Turks and Caicos Islands', code: 'TC', dial: '+1-649' },
  { name: 'Tuvalu', code: 'TV', dial: '+688' },
  { name: 'U.S. Virgin Islands', code: 'VI', dial: '+1-340' },
...
export function getCountryDial(code: string): string {
  return countries.find((c) => c.code === code)?.dial ?? '+1'
}

export function buildPhone(dial: string | undefined, localNumber: string | undefined): string {
  return `+${(dial ?? '+1').replace(/\D/g, '')}${(localNumber ?? '').replace(/\D/g, '')}`
}
